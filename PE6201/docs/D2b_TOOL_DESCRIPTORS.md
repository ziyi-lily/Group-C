...
The Problem A agent is equipped with seven tools. Each tool contains six required contract fields - **NAME + SIGNATURE, WHAT, INPUT, RETURNS, FAILS WHEN, and IRREVERSIBLE?** - and the number of returned tokens was collected by running the final fixture set (38 claims) through the o200k_base tokenizer and serializing it with repr(result). On September 14, 2026, we locked the final controlled measurement data using a deterministic scripted planner, parallel calls, a six-turn cap, and confirm autonomy.
...
| WHAT | Retrieves the claim to be assessed, including the member, hospital, service date, member-written narrative, attached documents, and all claim lines. It is the entry-point tool and must run before any tool that depends on claim fields. |
| INPUT | claim_id: str - an existing claim identifier such as CLM-9013. An unknown ID returns no record and indicates an inconsistent evaluation case rather than a business outcome. |
...
| WHAT | Track the relationship between members and policies and calculate the remaining annual limit as annual_limit - used_to_date. It provides the facts needed to check the policy status, the validity of service dates, exclusion clauses and remaining financial limits. |
...
| WHAT | Search for the treatment hospital and its panel status. Non-panel hospitals may change the interpretation and payment path, but they will not automatically determine the claim outcome. |
| INPUT | hospital_id: str - the hospital identifier returned by get_claim. An unknown ID returns no result. |
...
| WHAT | Resolves one claim line under one specific policy. It identifies the procedure, whether pre-authorisation is required, whether the policy excludes the procedure, the applicable exclusion rule, and any required document. It must be called once for every line; independent line calls may run in parallel. |
| INPUT | code: str - one procedure code from the claim; policy_id: str - the policy returned by lookup_policy. Both are mandatory. An unknown procedure or policy returns no result. |
...
| FAILS WHEN | The procedure code or policy does not exist, or a required data file is unavailable. An excluded procedure is not a tool failure: the tool returns excluded=true and the applicable rule. |
| IRREVERSIBLE? | No. Read-only. |
...
| INPUT | member_id: str, procedure_code: str, and date_of_service: str in YYYY-MM-DD format. All three values must match for a valid result. An approval for another member, another procedure, or a date outside the validity range does not authorise the claim. |
...
| FAILS WHEN | A required data file is unavailable or a supplied line lacks code or amount. is_duplicate=false is a normal result and processing must continue. |
| IRREVERSIBLE? | No. Read-only. |
...
| FAILS WHEN | The loop blocks the call before execution when the autonomy gate is not satisfied, and its duplicate-action guard stops an identical repeated call. The dispatcher returns a bad-arguments error when required arguments are missing or misnamed. |
...
The whole point of poka-yoke changes is to make mistakes structurally impossible instead of hoping the model will just be more careful.
...
Under the older design, check_coverage(code) could only answer at a global procedure level - policy-specific exclusions could easily slip through unnoticed. V2 closes this gap by requiring policy_id, which forces the tool to identify the relevant policy before returning a coverage result.


### 2.2 Return an explicit PA verdict and preserve expired evidence


Here's where things get interesting. V1 squashed two fundamentally different situations - "no matching PA exists" and "a matching PA exists but has expired" - into a single None. V2, on the other hand, hands back an explicit found flag and keeps a bounded expired_candidate around. That single change makes it impossible for the interface to quietly erase the existence, identifier, and validity dates of the most relevant expired PA.


### 2.3 Require all four duplicate facts and an explicit verdict


The duplicate tool now demands all four business facts - member, hospital, date of service, and the full line list - and returns an explicit is_duplicate boolean alongside separated exact_match and bounded near_matches. A near match can't get mistaken for an exact duplicate just because the return object happens to be non-empty.
...
FAILS WHEN
Returns None when no valid matching approval is found. None cannot distinguish
...
Sure, the one-token None result is compact. But it throws away evidence the model won't be able to reconstruct later.
...
The only things that differ between the two conditions are the get_preauthorisation descriptor and its return shape. Everything else - routing, hostile-input handling, answer-format instructions, and non-PA tool descriptions - stays exactly the same.
...
At the observed median of four turns, that bigger V2 prefix pumps in roughly 612 extra input tokens per run before you even count observation growth. It's a recurring cost, so the safety upside has to outweigh it.
...
Take the targeted expired-PA case CLM-8894 as an example: V1 handed back None in a single token. V2 returned the full structured result with the expired candidate PA-5640 in 68 tokens - a jump of 67 return tokens just for that one call.
...
Every single one of the ten scripted V1 failures happened right after get_preauthorisation on claims that actually carried valid PA evidence: CLM-8842, CLM-8861, CLM-9013, CLM-9015, CLM-9016, CLM-9020, CLM-9021, CLM-9165, CLM-9180, and CLM-9103. V2 brought those back in line by restoring compatibility with the working agent's structured PA handling.
...
V2 is bigger. It tacks on 18.85 mean return tokens per PA call, 264 total across the 14 measured PA calls, and roughly 45.0% more tokens on average for PA returns. That recurring observation cost is real and measurable, so it shouldn't be dressed up as some kind of token saving.


The deterministic regression increased from 82.8% to 100.0%, showing that the structured V2 result is compatible with the working agent and prevents valid PA records from being misread as missing by the scripted planner. Both versions passed all 12 guardrail cases, so the rewrite introduced no measured guardrail regression.


The live comparison did not show a broad code-check improvement: V1 passed 40/58 trials and V2 passed 39/58. The second-model judgement found a small semantic improvement, with the mixed pass rate moving from 5/58 to 7/58, but the absolute L2 rates remained low because many records omitted required evidence. These figures are reported without treating a one-trial code-check difference as a general model-quality result.


The targeted failure provides the clearest justification for the change. V1 erased the existence and dates of expired PA-5640 from the tool return. In the deterministic reproduction, V2 changed the required-evidence result from 0/3 to 3/3 without changing the decision or turn count. In the live battery, V1 recorded none of the three required PA facts in all three CLM-8894 trials; V2 recorded all three facts in one of three trials. Therefore, V2 is justified as an evidence-preserving interface constraint, not as a blanket accuracy or token-saving fix.
